        ...
        internal void ArrivalEvent()
        {
            if (ArrivedCount < Param.MaxClientCount)
            {
                ...
                if (CurrentClient == null)
                {
                    ...
                    (new DirectEvent(FinishedEvent)).ActivateDelay(RandCashman.Uniform(Param.MinCashTime, Param.MaxCashTime));
                }
                (new DirectEvent(ArrivalEvent)).ActivateDelay(RandClient.Exponential(Param.MeanClientInterval));
            }
        }
